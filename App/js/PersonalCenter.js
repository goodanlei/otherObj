$(function (){
	...
	$('.pup_sure').on('tap',function (){
		$('.per_del').hide();
		$(".delete[data-id='ok']").parents('.list').remove();
	});
	$('.pup_cancel').on('tap',function (){
		$('.per_del').hide();
	});
	...
	var istrue = null;
	//左滑
	$(document).on('swipeLeft','.list',function(){
	...
	$(document).on('swipeRight','.list',function(){
	...
	function Jump(){
		if(!istrue){
		//	alert(1);
    		var str = 'http://4.zyx1988.applinzi.com/gerenziyuan/viewResource.html';
